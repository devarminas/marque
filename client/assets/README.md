...
| `quaternius/bestiary/Imp.glb` | 1.678 | |
| `quaternius/bestiary/Puglin.glb` | 0.931 | |
...
| ARM-170, hand tools and weapons attach | `tools/`, `weapons/`, both held out of git |
| ARM-171, Nature MegaKit trees on gather nodes | `quaternius/nature/` |
| ARM-172, Imp and Puglin replace dummy NPC visuals | `quaternius/bestiary/`, held out of git |

## Staging manifest

Source packs live at `C:\Users\armin\Documents\Projects\game\assets\` and are not in git. Every file here is a byte-for-byte copy; two are copied under a different filename and say so below.

**Three directories are staged on disk but held out of git.** `quaternius/bestiary/`, `weapons/` and `tools/` are 26,287,720 bytes that this repository does not carry, because this repository is public and none of that material is CC0. Bestiary is under a licence with a redistribution clause; the Weapons and tool packs state no licence at all. ARM-178 settles it, and one `git add` of those three paths lands them the moment it does. Until then ARM-170 and ARM-172 have their meshes on the authoring machine and not in a clone. Everything else in this file describes them as staged, because they are, and because the numbers should not have to be re-derived later.

Bytes include the `.import` sidecars, which git also tracks. The full staged tree is 130,309,825 bytes, 124.27 MiB, across 113 files. Committed here is 104,022,105 bytes, 99.20 MiB, across 94 files. Regenerate the numbers with:

```powershell
$f = Get-ChildItem client\assets\quaternius, client\assets\weapons, client\assets\tools -Recurse -File
"{0} files {1} bytes" -f $f.Count, ($f | Measure-Object Length -Sum).Sum
```

### `quaternius/base_characters/` (15,496,981 bytes)
...
### `quaternius/animations/` (8,093,661 bytes)
...
### `quaternius/outfits_fantasy/` (70,192,185 bytes)
...
### `quaternius/bestiary/` (17,271,661 bytes)

From `Bestiary - Dungeon Monsters Kit[Standard]`. Quaternius Asset License v1.0, not CC0. Held out of git; see findings 2 and 4.

Taken: `Imp.glb`, `Puglin.glb`, `License_Standard.txt`. Both GLBs embed their textures, so the 20 MB of loose `T_Imp_*` and `T_Puglin_*` PNGs stayed in the bundle. The Standard pack ships only these two monsters.

### `quaternius/nature/` (10,239,278 bytes)
...
From `Weapons\gLTF\`. Licence unknown. Held out of git; see findings 2 and 3.
...
From `tool pack\`. Licence unknown. Held out of git; see findings 2 and 3.

Taken: `axe.obj` with `axe.mtl`, `pickaxe.obj` with `pickaxe.mtl`.

Left in the bundle: the `.fbx` versions of both, and the two shared 4096x4096 albedos `drewno_Albedo.png` (25.0 MB) and `metal_Albedo.png` (16.9 MB). The OBJ files carry the same meshes as the FBX files, so staging both would import each tool twice. The albedos never bind anyway. Both `.mtl` files and both `.fbx` files name them by an absolute path on the author's machine (`C:\Users\przem\Pictures\Desktop\tool pack1\...`), and the imported `axe.obj` mesh in `.godot/imported/` carries no texture path at all. ARM-170 rebinds materials by hand during conversion. A 4096x4096 albedo on a hand-held tool is the wrong resolution, so downscale before staging, and settle the licence in ARM-178 first.
...
2. **This repository is public.** `STANDING-ORDERS.md` line 15 calls it private; `gh api repos/devarminas/marque` returns `"visibility": "public"`. Every licence question below is decided against a public tree, not a private one, and one of the two documents is wrong.
3. Neither the local `Weapons` pack nor the local `tool pack` ships a licence file at any depth. Every other pack staged here does. Publishing art whose terms nobody knows is not defensible on any reading, so `weapons/` and `tools/` are held out of git. ARM-178 owns establishing the terms.
4. The Bestiary pack is under the Quaternius Asset License v1.0, not CC0. Section 2 grants commercial use with no credit required and explicitly covers contractors and collaborators. Section 3(a) forbids redistributing the assets themselves as an asset pack, in original or modified form, whether alone or bundled, and adds that this applies regardless of how much they were modified. A game's source tree is not an asset pack, and section 2 permits distributing a Product that incorporates the assets, so the reading that allows this is available. It is not a call to make unilaterally against a public repository, so `quaternius/bestiary/` is held out of git alongside the two unlicensed packs. Section 7 binds whichever licence version was in force when the pack was obtained, and the shipped text is dated 8/28/2026. The other Quaternius packs staged here (Universal Base Characters, UAL2, Modular Fantasy, Stylized Nature) are CC0 1.0 and carry no such clause.
...
6. The repo has no `.gitattributes` and no git LFS. This unit adds 124 MiB of binaries and later M8 work adds more. ARM-179 owns the evaluation. Nothing here enables it.
