...
func _test_present_at_places_and_toggles_walk_anim() -> void:
	...
	_assertions.check_position_near(
		_ground(avatar), Vector2(0.0, 0.0), POSITION_EPSILON, "teleport lands at the pose"
	)

	avatar.present_at(3.0, -1.0, true)
	_assertions.check_position_near(
		_ground(avatar), Vector2(3.0, -1.0), POSITION_EPSILON, "present_at places the body"
	)
	var animation := avatar.get_node_or_null("AnimationPlayer") as AnimationPlayer
	_assertions.check(
		animation != null and animation.current_animation == PlayerAvatar.WALK_ANIM,
		"walking present_at plays %s, got \"%s\""
...
func _test_two_avatars_do_not_share_state() -> void:
	...
	second.present_at(0.0, -2.0, true)

	_assertions.check(first.player_id == 11 and second.player_id == 12, "ids stay distinct")
	_assertions.check_position_near(
		_ground(first), Vector2(2.0, 0.0), POSITION_EPSILON, "the first avatar is east"
	)
	_assertions.check_position_near(
		_ground(second), Vector2(0.0, -2.0), POSITION_EPSILON, "the second is north"
	)
...
func _test_an_unmoved_avatar_idles() -> void:
	...
	var animation := avatar.get_node_or_null("AnimationPlayer") as AnimationPlayer
	avatar.present_at(0.0, 0.0, false)
	_assertions.check(
		animation != null and animation.current_animation == PlayerAvatar.IDLE_ANIM,
		"an unmoved avatar idles on %s, got \"%s\""
		% [PlayerAvatar.IDLE_ANIM, "" if animation == null else animation.current_animation],
	)
	avatar.queue_free()
