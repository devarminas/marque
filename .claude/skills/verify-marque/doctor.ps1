...
try {
    # Captured through a pipeline on purpose. `$v = godot --version` reads
    # back empty (as do `@(...)`, `(...)`, `$(...)`), while `| Out-String`,
    # `| ForEach-Object`, and `| Select-Object -First 1` capture the version
