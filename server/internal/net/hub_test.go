...
func TestDisconnectDespawns(t *testing.T) {
	...
	// A clean logout is logged with its cause, not a detector name. This is the
	// only place the net layer's classification is checked end to end through
	// the world; net's own tests stop at the event.
...
func TestConcurrentTrafficStaysConsistent(t *testing.T) {
	...
	// Only the test's own goroutine may fail a test; background goroutines
	// report here instead, and this goroutine fails on their behalf.
	bg := newBackgroundErr()
	...
	// Sending to a connection nobody drains any more must not deadlock the tick.
	wg.Add(1)
	go func() {
		defer wg.Done()
		for {
