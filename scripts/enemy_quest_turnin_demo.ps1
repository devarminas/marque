...
$work = Join-Path ([System.IO.Path]::GetTempPath()) ("marque-enemy-quest-turnin-" + [guid]::NewGuid().ToString("n"))
...
try {
    $null = Initialize-MarqueEvidenceDir -OutDir $OutDir -SourceScript "scripts/enemy_quest_turnin_demo.ps1"
    ...
    foreach ($spec in $specs) {
        ...
        $godotArgs = @(
            ...
            "--",
            "--server", $url,
            "--enemy-quest-turnin-shots", ('"' + $spec.Prefix + '"'),
            "--enemy-quest-turnin-role", $spec.Role
        )
    ...
    foreach ($client in $clients) {
        ...
        # Shot map: 1 accept, 2 killsready (5/5), 3 complete with reward kinds.
...
Write-MarqueDemoResult `
    -OkMarker "ENEMY QUEST TURNIN DEMO OK" `
    -FailMarker "ENEMY QUEST TURNIN DEMO FAILED" `
