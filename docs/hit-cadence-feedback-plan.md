...
- [ ] Lane 2. Player to NPC white hit. Run `cd server && CGO_ENABLED=1 go test -v -count=1 -run TestSwing ./internal/game` and lift the player to NPC swing line out of the run, which the presentation suite logs as `WIRE <index> <body>` off a real websocket. Save `/home/armin/Work/marque-program/receipts/arm-311/lane-2-player-npc-frame.txt`. Pass when the captured frame carries a positive `amount` with `crit=false` and `miss=false`, and the presentation protocol suite decodes that same frame shape. The client stdout cannot show an amount until the ARM-315 consumer lands.
- [ ] Lane 3. NPC to player white hit. Run `enemy_midchase_demo.ps1` until an Imp lands a white, and read the camp Imp that aggros in the `dummy_attack_demo.ps1` run. Save `/home/armin/Work/marque-program/receipts/arm-311/lane-3-npc-player.log`. Pass when the GAMELOG and the wire carry an amount on the Imp swing.
...
- [ ] Lane 8. Swing still precedes HP. Run `cd server && CGO_ENABLED=1 go test -v -count=1 -run TestSwing ./internal/game`, which logs the frame index for all three swing sites, and keep the `dummy_attack_demo.ps1` run for the live layer. Save `/home/armin/Work/marque-program/receipts/arm-311/lane-8-ordering.log`. Pass when the swing frame index is below every HP frame for the same target in the run.
- [ ] Lane 9. Wire table. Run `cd server && CGO_ENABLED=1 go test -v -count=1 -run TestEncodeProducesKeyAsTagEnvelope ./internal/net`. Save `/home/armin/Work/marque-program/receipts/arm-311/lane-9-protocol-table.log`. Pass when the exact JSON table passes for both frames.
