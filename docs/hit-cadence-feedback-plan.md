...
- [ ] Lane 2. Player to NPC white hit. Run `dummy_attack_demo.ps1` and capture the swing frame off the running server's own socket, which is what the presentation suite logs as `WIRE <index> <body>` against a real server and client pair. Save `arm-311-lane-2-player-npc.png`. Pass when the captured frame carries a positive `amount` with `crit=false` and `miss=false`, and the presentation protocol suite decodes that same frame shape. The client stdout cannot show an amount until the ARM-315 consumer lands.
