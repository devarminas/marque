...
- [ ] Decode the new fields in `net_client.gd` `_on_swing` and `_on_cast_phase` and pass them on two new signals, `swing_hit_observed` carrying `amount`, `crit`, and `miss`, and `cast_effect_observed` carrying `amount` and `effect`. Godot 4.7 errors when a signal emits more arguments than its connected callable takes, so extending `swing_observed` and `cast_phase_observed` would break every existing connection. Touch no client logic.

**You see.**

- [ ] A player swing frame reads exactly `{"swing":{"id":1,"target":1000005,"weapon":"sword","amount":7,"crit":false,"miss":false}}`, the bytes `protocol_test.go` pins. The wire is struct ordered, so the keys arrive in that order and not alphabetised.
- [ ] A fireball resolve frame reads exactly `{"cast_phase":{"id":1000005,"ability":"fireball","target":1,"phase":"resolve","amount":22,"effect":"damage"}}`, also the bytes `protocol_test.go` pins. A begin and a cancel frame read as they do at trunk, because `amount` and `effect` carry `omitempty`.
...
- [ ] Lane 1. Regression lane against trunk. Run `dummy_attack_demo.ps1` and `cast_bar_demo.ps1` at head. Trunk has no `amount` key, so record the trunk gap from the trunk source instead of running it. Save `/home/armin/Work/marque-program/receipts/arm-311/lane-1-regression.log`. Pass when both demos print their OK marker at head and the trunk struct is recorded as lacking the fields.
...
- [ ] Blocking. Restore the miss pin that ARM-311 dropped from `server/internal/game/combat_test.go`, where `TestAttackHitLogsTheCritFlag` replaced `hits[0]["miss"] != false` with a comment pointing here. The restored case must fail while `miss` is still a hardcoded `false` at every emit site, which the 100 percent miss case does.
...
- [ ] Blocking. Consume `swing_hit_observed` and `cast_effect_observed` in production. Both are emitted and connected nowhere at the ARM-311 head, so they are dead code until this PR routes them. A resolve frame with no `amount` key renders no float rather than a zero, which ADR 0013 states and `client/tests/test_presentation_protocol.gd` pins.
