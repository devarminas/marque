...
2. **White damage roll.** The server checks the white miss chance before damage and crit. A miss uses one random draw, deals zero damage, reports `miss=true` and `crit=false`, and sends no HP frame. The world knob defaults to zero, which skips the miss draw; marqued defaults to 5 percent through `-white-miss-pct`. On a hit, a swing rolls uniformly in `[damage_min, damage_max]`, then adds the attacker's `AP` (`(STR - 10) / 2`), then checks `CritChance` (`max(0, DEX - 10)` percent) to double the pre-armor amount. Finally the target's `Armor` (`max(0, (DEX - 10) / 2)`) is subtracted with a floor of 1. Misses never use the armor floor. Abilities and heals do not use this knob.
