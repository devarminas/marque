...
1. **Four downlink messages.** The server broadcasts each one to every connected client.
   ...
   | `cast_phase` | `{id, ability, target, phase, amount, effect}` | `phase` is `begin` when a timed cast starts, `resolve` when an ability applies, and `cancel` when a pending cast ends without applying. A `resolve` carries `amount` when the target actually moved, how much HP it gained or lost after its clamp, and `effect`, the ability's effect kind (`heal` or `damage`). A resolve that moved no HP omits `amount`, and an absent `amount` means the applied delta was zero. A client reads it as no number to render, never as a zero to display. `begin` and `cancel` carry neither, so they keep their earlier bytes. Players and NPCs both send it. |
