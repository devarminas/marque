...
Accepted (M13c). Settles the JSON names sketched in `docs/adr/0001-movement-authority.md`.
...
5. **Player `path`.** Retired for player locomotion and approach (M13g). Gather, talk, attack, and pickup out of range use sticky steer + `pose`; in range is a range check. NPC `path` / polyline walking may remain.
